import {STUInt, makeStrongUInt, makeStrongUIntNB} from '../../src/types/uint';
...
describe('STUInt', () => {
	describe('Implementation', () => {
		...
		it('should not set value when called with a negative value', () => {
			...
			const uint = makeStrongUInt(null, sampleFallbackDefault);
			uint(negativeInt);
...
describe('STUIntNB', () => {
	describe('Implementation', () => {
		it('should return null when value is not set', () => {
			const sampleFallbackDefault = 3900001;
			const uint = makeStrongUIntNB(null, sampleFallbackDefault);
		...
		it('should return null after value has been set to null', () => {
			...
			const uint = makeStrongUIntNB(sampleInt, sampleFallbackDefault);
			uint(null);
			expect(uint()).toBeNull();
		});
	});
	...
	describe('Methods', () => {
		describe('get', () => {
			...
			it('should return fallback after previous value is overwritten with null', () => {
				...
				const uint = makeStrongUIntNB(sampleInit, sampleFallbackDefault);
				uint(null);
				expect(uint.get(sampleFallbackDefault)).toBe(sampleFallbackDefault);
			});
		});
