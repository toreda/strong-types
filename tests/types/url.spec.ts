...
describe('StrongUrl', () => {
	describe('Implementation', () => {
		it('should set initial value to sampleInitial argument', () => {
			...
			const result = makeUrl(sampleInitial, MOCK_FALLBACK_DEFAULT);
		...
		it('should not set value when called with an empty string', () => {
			...
			const emptyString = '';
			const result = makeUrl(null, sampleFallback);
			result(emptyString);
		...
		it('should return fallback default when value is undefined', () => {
			...
			const result = makeUrl(undefined, sampleFallback);
			expect(result()).toBe(sampleFallback);
		});
	...
	describe('Methods', () => {
		describe('get', () => {
			it('should return fallback argument when value is null', () => {
				...
				const string = makeUrl(null, MOCK_FALLBACK_DEFAULT);
			...
			it('should return value when value is set', () => {
				...
				const string = makeUrl(sampleInitial, MOCK_FALLBACK_DEFAULT);
